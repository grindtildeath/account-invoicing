...
class AccountInvoice(models.Model):
    ...
    def _swedish_round_globally(self, invoice, rounded_total, delta):
        ...
        if ajust_line:
            ...
            vals = inv_tax_obj.amount_change([ajust_line.id], amount,
    ...
    def _compute_swedish_rounding(self, invoice):
        ...
        if round_method == 'swedish_add_invoice_line':
            ...
            #                                       rounded_total, delta,
            #                                       context=ctx)
            return self._swedish_add_invoice_line(invoice, rounded_total, delta)
        elif round_method == 'swedish_round_globally':
            # return self._swedish_round_globally(cr, uid, invoice,
            #                                     rounded_total, delta,
            #                                     context=ctx)
            return self._swedish_round_globally(invoice, rounded_total, delta)
        return {}
        ...
        if taxes and taxes[0].company_id.tax_calculation_rounding_method[:7] \
            ...
            if not precision:
                precision = self.env['decimal.precision'].precision_get(
                    'Account')
            precision += 5
        return super(AccountTax, self).compute_inv(taxes, price_unit, quantity,
                                                   product=product,
                                                   partner=partner,
                                                   precision=precision)

    @api.model
    def _compute(self, taxes, price_unit, quantity, product=None, partner=None,
                 precision=None):
        """Using swedish rounding we want to keep standard global precision
        so we add precision to do global computation
        """
